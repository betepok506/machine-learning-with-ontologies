...
# mymodel = gensim.models.Word2Vec.load("opa2vec/RepresentationModel_pubmed.txt")
path_to_model = f"{pretrain}"
if not os.path.exists(path_to_model) or not os.path.exists(path_to_model + '.syn1neg.npy') or not os.path.exists(path_to_model + ".wv.syn0.npy"):
	print(f"Необходимо скачать файлы RepresentationModel_pubmed.txt, \
	RepresentationModel_pubmed.txt.syn1neg.npy, RepresentationModel_pubmed.txt.wv.syn0.npy!")
	exit(0)
	
mymodel = gensim.models.Word2Vec.load(path_to_model)
